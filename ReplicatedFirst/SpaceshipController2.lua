...
local spaceship = ReplicatedStorage:WaitForChild("Spaceship2")
...
game:GetService("RunService").Heartbeat:Connect(function(deltaTime)	
	if workspace.Pause.Value == false and workspace.curStage.Value~=2 and workspace.curStage.Value<6 then
		...
		if spawnTimer >= spawnCool then -- if 5 seconds have elapsed since the last spaceship was spawned
			...
			local position1 = Vector3.new(math.random(-250, -150), 70, -510)
			...
			for i, v in pairs(spaceshipInstance1:GetChildren()) do --몬스터안에 파트 구하기
				if v.ClassName == "Part" then --파트라면(휴머노이드나 아이템 제외하기 위해)
					v.Touched:Connect(function(hit) --파트에 무언가 닿이면
						if(hit.Parent ~= nil and hit.Parent.Name == "Spaceship2") then
							return
						end
						if(hit.Parent.Name ~= nil and hit.Parent.Name ~= "Spaceship2") then
